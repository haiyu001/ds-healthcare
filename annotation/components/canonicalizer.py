...
def _get_similarity(unigram: str, bigram: str, wv_model: Union[FastTextKeyedVectors, KeyedVectors]) -> Optional[float]:
...
def _get_canonical(unigram: str, bigram: str, unigram_count: int, bigram_count: int,
...
def get_bigram_norm(bigram_norm_candidates_filepath: str,
    ...
    pdf["similarity"] = pdf.apply(lambda x: _get_similarity(x["unigram"], x["bigram"], wv_model), axis=1)
    pdf = pdf.dropna(subset=["similarity"])
    pdf["canonical"] = pdf.apply(lambda x: _get_canonical(x["unigram"], x["bigram"], x["unigram_count"],
...
if __name__ == "__main__":
    ...
    bigram_norm_filepath = os.path.join(domain_dir, canonicalization_folder,
                                                   annotation_config["bigram_norm_filename"])
    ...
    wv_model_filepath = os.path.join(domain_dir, canonicalization_folder, canonicalization_wv_folder, "model", "fasttext")
